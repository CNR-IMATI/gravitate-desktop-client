...
namespace GravitateMetadata
{

ModelResolution findTargetResolution(PropertyType type)
{
    using GravitateRepo::PropertyResolutionMapping;
    using GravitateRepoUtils::PropertyFileMap;

    for(auto entry: PropertyResolutionMapping)
    {
        auto prop = entry.first;
        auto res = entry.second;
        if(prop == PropertyFileMap.value(type))
        {
            return res;
        }
    }
